...
module AhoyAnalytics
  class ReferrersController < AhoyAnalytics::BaseController
    def index
      ...
      if source.casecmp("Google").zero?
        ...
        payload = cache_for([ :search_terms_alias, limit, page, search, params[:order_by] ]) do
          search_terms_payload(@query, limit:, page:, search:)
        end
        return render json: camelize_keys(payload), status: :ok
      end
